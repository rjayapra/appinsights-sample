...
dataServiceUri=$(az webapp show -g $rg -n $baseName-data  --query "hostNames[0]"  | tr -d '"')
...
itineraryServiceUri=$(az webapp show -g $rg -n $baseName-itinerary  --query "hostNames[0]"  | tr -d '"')
